...
public class SearchvoiceController {
     ...
     */
	 ...
	 public static ArrayList<String> prepareUrls(String stage_base_url, String
	 live_base_url, String keyword) { ArrayList<String> urls = new ArrayList<>();
	// keyword = keyword.replaceAll("\\s", "%20"); //

	urls.add(GlobalConfigHandler.encodeUrl(stage_base_url+Endpoints.autoSuggestvoicesearchstageEndpoint(keyword)));
	urls.add(GlobalConfigHandler.encodeUrl(live_base_url+Endpoints.autoSuggestvoicesearchliveEndpoint(keyword)));
	urls.add(GlobalConfigHandler.encodeUrl(stage_base_url+Endpoints.autoSuggestvoicesearchsolrEndpoint(keyword)));
	System.out.println("Searchvicecontroller> endpoints for voice search"); 
	return urls;
