...
import logic_controller.SearchvoiceController;
import utils.Mailer;
...
public class SearchVoice {
    ...
    public void prepareTestEnv(){
        ...
        KEY_INPUTS = SearchvoiceController.getTestData(API_NAME+".csv");
    ...
    public void createGetRequestAutoSuggestCall(String keyword){
        if(API_CALL == 1)
            getLiveBaseUrl();
        ArrayList<String> urls = SearchvoiceController.prepareUrls(STAGE_BASE_URL, LIVE_BASE_URL, keyword);
      //  ArrayList<String> urls = SearchvoiceController.prepareUrls(prop, keyword);
